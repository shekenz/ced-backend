...
">
	...
	<div><a href="{{ route('messages') }}" class="base-menu-link {{ (request()->routeIs('contact')) ? 'base-menu-link-active' : '' }} base-menu-animated">contact</a></div>
	@if(request()->user() || config('app.env') == 'local')
		<div class="md:col-start-8 "><a href="{{ route('cart') }}" class="{{ (request()->routeIs('cart')) ? 'base-menu-link-active ' : '' }}base-menu-link base-menu-animated">cart{{ (CartHelper::isEmpty()) ? '' : ' ('.CartHelper::count().')'  }}</a></div>
